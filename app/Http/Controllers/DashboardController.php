...
use App\Models\{CorrectiveAction,Incident,Inspection,Risk,ActivityLog};
class DashboardController extends Controller {
 public function __invoke(){
  ...
  return view('dashboard.index',compact('stats'),['actions'=>$actions->with('assignee','department')->orderBy('due_date')->limit(7)->get(),'recentRisks'=>$risks->with('department')->latest()->limit(5)->get(),'upcomingInspections'=>Inspection::with('department','inspector')->whereIn('status',['planned','in_progress'])->when($department,fn($q)=>$q->where('department_id',$department))->when($u->hasRole('inspector'),fn($q)=>$q->where('inspector_id',$u->id))->orderBy('scheduled_at')->limit(5)->get(),'activities'=>$u->hasRole(['admin','hse_manager'])?ActivityLog::with('user')->latest('created_at')->limit(6)->get():ActivityLog::with('user')->where('user_id',$u->id)->latest('created_at')->limit(6)->get()]);
 }
}
