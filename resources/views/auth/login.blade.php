            ...
            <form method="post" action="{{ route('login.store') }}" class="space-y-5" novalidate>
                ...
                <div>
                    <label for="email" class="mb-2 block text-sm font-bold
                        {{ $emailError ? 'text-rose-600' : 'text-slate-800' }}">
                        ایمیل سازمانی
                    </label>
                    <input
                        id="email"
                    ...
                    >
                    @if($emailError)
                        <p class="mt-1.5 text-xs text-rose-600">{{ $emailError }}</p>
                    @endif
                </div>

                {{-- ── Password field ───────────────────────────── --}}
                @php $passwordError = $errors->first('password') @endphp
                <div>
                    <label for="password" class="mb-2 block text-sm font-bold
                        {{ $passwordError ? 'text-rose-600' : 'text-slate-800' }}">
                        رمز عبور
                    </label>
                    <input
                        id="password"
                    ...
                    >
                    @if($passwordError)
                        <p class="mt-1.5 text-xs text-rose-600">{{ $passwordError }}</p>
                    @endif
                </div>
