...
<div class="min-h-screen lg:grid lg:grid-cols-[270px_1fr]">
 <aside class="hidden lg:flex sticky top-0 h-screen flex-col bg-slate-950 text-white">
  ...
  <nav class="scrollbar-thin flex-1 space-y-1 overflow-y-auto p-4 text-sm">
   ...
   @if(auth()->user()->hasRole(['admin','hse_manager']))<div class="px-3 pb-1 pt-5 text-[10px] font-bold tracking-widest text-slate-500">مدیریت و تحلیل</div><a href="{{ route('checklists.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-300 hover:bg-white/5 hover:text-white">@include('components.icon',['name'=>'list']) چک‌لیست‌ها</a><a href="{{ route('reports.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-300 hover:bg-white/5 hover:text-white">@include('components.icon',['name'=>'chart']) گزارش‌های مدیریتی</a>@endif
   @if(auth()->user()->hasRole('admin'))<a href="{{ route('users.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-300 hover:bg-white/5 hover:text-white">@include('components.icon',['name'=>'users']) کاربران و دسترسی‌ها</a><a href="{{ route('departments.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-300 hover:bg-white/5 hover:text-white">@include('components.icon',['name'=>'building']) واحدهای سازمانی</a><a href="{{ route('settings.edit') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-300 hover:bg-white/5 hover:text-white">@include('components.icon',['name'=>'settings']) تنظیمات سامانه</a>@endif
  </nav>
